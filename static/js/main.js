...
function enableReaction() {
  ...
  const seed = { "👍": [0, false], "👀": [0, false], "🤔": [0, false] };
  const storageKey = `serene-reaction:${slug}`;
  ...
  const writeLocal = () => {
    localStorage.setItem(storageKey, JSON.stringify(state.reaction));
  };

  const useLocal = endpoint === 'local' || !endpoint || endpoint.includes('example.com');
